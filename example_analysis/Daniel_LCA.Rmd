...
fit_table <- table_fit(res)
fit_table[ , c("Name", "LL", "AIC", "BIC", "Entropy", "prob_min", "prob_max", "n_min", "n_max")]
```
...
```{r}
library(ggplot2)
plot_prob(res[[3]]) + scale_fill_manual(name = "bla")
table_prob(res[[3]])
```
...
```{r}
probs <- class_prob(res[[3]])
probs$mostlikely.class
probs$individual
```
