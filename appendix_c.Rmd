---
title             : "Appendix C: Latent Class Growth Analysis"
...
part of the *Prospection in Stress-related Military Research (PRISMO)*
...
papaja::apa_table(desc, caption = "Item descriptives")
```
...
p = ggplot(df_plot, aes(x = scl)) +
  geom_density() +
  facet_wrap(~time) + theme_bw()
ggsave("appendices/plot_dist.png", p, width = 150, height = 120, units = "mm")
```

```{r, echo = FALSE, out.width="50%", eval = pkgdown}
knitr::include_graphics("appendices/plot_dist.png")
```
...
p <- ggplot(df_plot, aes(x = Value, colour = Transformation)) +
  ...
  xlab("scl (rescaled to 0-1)") +
  theme_bw()
ggsave("appendices/plot_trans.png", p, width = 150, height = 120, units = "mm")
```

```{r, echo = FALSE, eval = pkgdown, out.width="50%"}
knitr::include_graphics("appendices/plot_trans.png")
```
...
# convergence problems for model 1:
res_step[[1]] <- mxTryHardWideSearch(res_step[[1]], extraTries = 50)
saveRDS(res_step, "appendices/res_step.RData")
fit <- table_fit(res_step)
saveRDS(fit, "appendices/lcga_fit.RData")
```

```{r, echo = FALSE, eval = FALSE}
res_step <- readRDS("appendices/res_step.RData")
```
...
find a better solution, using `OpenMx::mxTryHardWideSearch()`. This also
illustrates that `tidySEM` mixture models inherit from `OpenMx`'s
`MxModel`, and thus, different `OpenMx` functions can be used to act on
models specified via `tidySEM`.
...
# Select columns
tab_fit <- tab_fit[, c("Name", "Classes", "LL", "Parameters", "BIC", "Entropy", "prob_min", "n_min", "warning", "lmr_p")]
tab_fit
```

```{r tabfit, eval = TRUE, echo = FALSE, results = "asis"}
# Get fit table fit
tab_fit <- readRDS("appendices/lcga_fit.RData")
tab_fit <- tab_fit[, c("Name", "Classes", "LL", "Parameters", "BIC", "Entropy", "prob_min", "n_min", "warning", "lmr_p")]
names(tab_fit) <- c("Name", "Classes", "LL", "p", "BIC", "Ent.", "p_min", "n_min", "warn", "lmr_p")
papaja::apa_table(tab_fit, digits = 2, caption = "Fit of LCGA models")
```
...
ggsave("appendices/lcga_plot_fit.png", p, width = 150, height = 120, units = "mm")
```

```{r echo = FALSE, eval = pkgdown, out.width="50%"}
knitr::include_graphics("appendices/lcga_plot_fit.png")
```
...
tab_res <- read.csv("appendices/lcga_tab_res.csv", stringsAsFactors = FALSE)
class(tab_res) <- c("tidy_results", "data.frame")
tab_res <- tab_res[tab_res$Category %in% c("Means", "Variances"), c("Category", "lhs", "est", "se", "pval", "confint", "name")]
papaja::apa_table(tab_res, digits = 2, caption = "Results from 3-class LCGA model", longtable = TRUE)
```
...
wald_tests <- read.csv("appendices/lcga_wald_tests.csv", stringsAsFactors = FALSE)
...
labs <- round(invbc(scales::rescale(brks, from = c(0, 1), to = rng_bc), lambda))
p <- p + scale_y_continuous(breaks = seq(0, 1, length.out = 5), labels = labs) + ylab("SCL (rescaled from Box-Cox)")
ggsave("appendices/plot_traj.png", p, width = 150, height = 120, units = "mm")
```

```{r echo = FALSE, eval = pkgdown, out.width="50%"}
knitr::include_graphics("appendices/plot_traj.png")
```
