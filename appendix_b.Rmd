---
title             : "Appendix B: Exploratory LPA for Ocean Microplastics"
...
This is an example of exploratory latent class analysis (LCA) with continuous indicators, otherwise known as latent profile analysis (LPA) or finite Gaussian mixture modeling, using `tidySEM`.
...
To view its documentation, run the command `?tidySEM::alkema_microplastics` in the R console.
The original analyses are available at <https://github.com/cjvanlissa/lise_microplastics>; in this vignette, we take a different approach to the analysis to showcase other possibilities.
...
There are no missing values; if any variables had missing values, we would report an MCAR test with `mice::mcar()`, and explain that missing data are accounted for using FIML.
...
As all variables are continuous, we can use the convenience function `tidySEM::mx_profiles()`, which is a wrapper for the generic function `mx_mixture()` optimized for continuous indicators.
...
All variables in `data` are included in the analysis, which is why we first selected the indicator variables.
...
fit$par_ratio <- (5605*fit$n_min) / (fit$Parameters/fit$Classes)
tbl <- fit[ , c("Name", "LL", "Parameters", "par_ratio",
         "BIC", "Entropy",
         "prob_min",
         "n_min",
         "lmr_p")]
names(tbl) <- c("Name", "LL", "p", "par_ratio",
         "BIC", "Ent.",
         "p_min",
         "n_min",
         "lmr_p")
...
If we omit the 1-class models, for which entropy is technically not defined, the correlation is even as high as .85, `cor(fit$BIC[!fit$Classes == 1], fit$Entropy[!fit$Classes == 1])`.
...
If our strategy is to optimize fit, we can examine the fit table above, or plot a scree plot for the BIC by calling `plot(fit)`.
...
Out of the 16 models that remain after removing those with \< 10% of cases in the smallest class, one model stands out: The 2-class model with free (co)variances.
<!-- The function `ic_weights(fit)` allows us to compute IC weights for all models in the set; --> <!-- it prefers the most complex model out of the 16 models with a posterior model probability of nearly 100%. --> We thus select this as our final model.
...
We can simply label class 1 as *small fragments*, and class 2 as *larger fragments*.
...
There are, however, concerns about the interpretability of this solutions: the entropy is `.56` and the minimum classification probability is`.81`.
...
The `BCH()` function applies three-step analysis, which compares the classes using a multi-group model, controlling for classification error.
...
To obtain an omnibus likelihood ratio test of the significance of the differences in polymer type across classes, use `lr_test(aux_pt)`.
