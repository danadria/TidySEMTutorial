...
membership. The concept of LCA can be understood in different ways.
A mixture model assumes that the study population is composed of $K$
...
Another way to conceptualize latent class analysis is by analogy to a measurement model in structural equation modeling [@molenaarArbitraryNatureLatent1994].
...
on par with multiple imputation when the
estimation model includes the same variables as the imputation model
[@lee_comparison_2021].
Although FIML does not, by
default, handle missingness in exogenous variables - all indicator
variables in LCA are endogenous, so this is not a concern for LCA.
Multiple imputation, a common alternative to FIML,
is less suitable to LCA for two reasons. First,
...
For this tutorial,
the lead author has implemented this test in the `mice` package as `mice::mcar()`, with a
fast backend in C++ and new printing and plotting methods.
Our recommendation is to inspect missingness and report the proportion of missingness per variable using `tidySEM::descriptives()`,
and testing for MAR using `mice::mcar()`,
before proceeding with FIML.

One minor concern is that the K-means algorithm, which
`tidySEM` uses to determine starting values, is *not* robust to
partially missing data [@hartiganAlgorithm136KMeans1979].
When missing data causes it to fail,
`tidySEM` automatically switches to hierarchical clustering [@murtaghWardHierarchicalAgglomerative2014].
The user can also specify a different
clustering algorithm or specify starting values manually.
Cases with complete missing data should be removed,
or their data should be imputed another way.

### Model Specification

Before estimating a LCA model, researchers should carefully specify the model,
...
the number of latent classes and the parametrization within each class.
The total number of parameters of a latent class model consists of the number of parameters within each class times the number of classes,
plus a classification vector with $K-1$ free elements.
Different types of latent class models have different parameters.
For example, latent profile analyses typically assume class-specific means,
...
LCA with ordinal indicators typically assume free thresholds across classes,
where each threshold reflects a proportion of responses within a particular response category, see `mx_lca()`.
Like latent profile analyses, latent class growth analyses typically have free means across classes, but with respect to the latent growth variables instead of the observed variables, see `mx_growth_mixture()`.
In the general sense, any structural equation model can be specified,
either using `lavaan` model syntax or using `OpenMx` RAM specification,
and supplied to the general function `mx_mixture()`.
The resulting model may have means, (co)variances, regression coefficients, factor loadings, and thresholds for ordinal variables.
All of these parameters can be freely estimated,
constrained to be equal across classes using equality constraints (e.g., `x ~~ a*x` fixes the variance of `x` to be equal across classes),
or fixed to a certain value (e.g., `x ~.5*1` fixes the mean of `x` to `.5`).

As any of these parameters could be misspecified,
prior literature has emphasized the importance of considering alternative model specifications [@van_de_schoot_grolts-checklist_2017].
While we agree with this recommendation,
two considerations have remained underemphasized in prior literature.
One consideration is that alternative model specifications may be approached differently depending on whether the LCA is data driven (exploratory), or theory driven
(confirmatory).
The section on Use Cases illustrates these different applications.

Prior literature has focused on exploratory LCA.
In this context, model specification typically consists of an exhaustive grid search along varying numbers of classes and model specifications.
The "correct" model specification is then determined based on a combination of fit indices, significance tests, and interpretability [@nylund_deciding_2007].
Confirmatory LCA does not require such an extensive search over the model space.
Theoretical models can be compared to a smaller set.
If hypothesized models have been specified a priori,
and possibly even preregistered,
it makes sense to focus on the estimation and interpretation of these models, rather than proceed in a purely data-driven manner.
One key challenge is that LCA lacks objective fit indices,
and the fit of a theoretical model can thus only be determined relative to other likely candidates.
...
A theoretical model would be supported by a substantial (and significant) difference in fit with a one-class model.
...
Here again, a substantial difference in fit supports the theoretical model - but note that small differences in fit do not give cause to reject a theoretical model.
...
The BIC is the preferred fit index for comparing 
...
The second consideration is the risk of overfitting.
We address each in turn.

As explained before, the total number of parameters scales with the number of estimated classes.
