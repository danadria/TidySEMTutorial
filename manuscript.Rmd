...
For instance, the aforementioned study on marine plastic noted that many prior studies had treated the distribution of particles as (multivariate) normal, despite clear deviations from normality [@alkema_maximizing_2022].
...
By default, `tidySEM` derives starting values using K-means clustering [@likas_global_2003],
...
$\ln(\frac{n + 2}{24})$. Although simulation studies have showen that the saBIC performs well for class enumeration [@tein_statistical_2013; @chen_efficacy_2017],
...
By default, `table_fit()` reports the VLM for subsequent models in the table.
