...
[@wu_abuse_2011] problems, or health outcomes after recovering from
cancer [@hardy_exploring_2018]. The example of shoe size is a rudimentary
illustration of this type of application. As another example, a recent study 
identified two suicide risk profiles in military service members and veterans
[@gromatsky_characteristics_2022].  One class was defined by higher-risk
history and another by lower-risk history. The higher-risk group comprised of
individuals with greater intent to die using methods with higher lethality,
and the lower-risk group was characterized by greater ambivalence towards death
and usage of lower lethality methods.

A related use case is where class-membership is known but imperfectly measured.
Mixture models can also be useful to evaluate the quality of measurement
across instruments. For example, a recent study had employment status 
...
The authors used a mixture model to evaluate the underlying factor of 
...
distinct risk profiles in the Winnipeg area. Another study investigated 
adolescent empathy development and its influence on parent-child conflict 
[@van_lissa_divergence_2015]. The data was collected over six years. 
Using LCGA, adolescents with similar empathy development trajectories 
were grouped into: high-, average-, and low-empathy classes.
...
indicators are treated as continuous [e.g., @norman_likert_2010] or 
with 5+ categories [@rhemtulla_when_2012], this
certainly does not imply that all methods are. It is certainly unlikely
that such ordinal variables can be treated as a *mixture* of multiple
