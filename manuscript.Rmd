...
In LCA, a sequence of models is fitted to the data with each additional model having one more class than the previous model.
The final model called the final class solution is chosen based on both theoretical and statistical criteria.
Theory should inform the choice of indicator variables ... 
With regards to statistical criteria, model fit is often assessed through multiple information criteria.
...
Other statistical criteria include the Bayes factor and likelihood ratio tests
which can also be used to assess model fit (Weller, Bowen & Faubert, 2020).
...
instill confidence that the estimation was successful. 
...
Once we have selected the final class solution, 
<!-- Continue the paragraph with info about classification diagnostics, e.g. the average latent class posterior probability. -->

Another consideration has to do with conditional item probabilities which indicate 
the probability of a positive score on an item given that the person belongs to a particular latent class. 
If many conditional item probabilities in estimation are found to be extreme (as in exactly 0 or 1), 
