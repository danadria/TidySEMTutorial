...
Each set of starting values will result in a model with a log likelihood value. Ideally, the largest log likelihood value will be replicated a large number of times (each time is generated by a different starting value set). Successful replications of the largest log likelihood value across different starting values instill confidence that the estimation was successful. The model with the highest replicated likelihood will be chosen as our best-fit. If the largest log likelihood value is not replicated many times, the model may be unidentified (Geiser, 2013).

[to add: Nylund-Gibson & Choi  p.22. about 3-10% and Geiser p. 237 -240] 
