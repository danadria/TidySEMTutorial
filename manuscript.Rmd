...
abstract: |
  ...
  using free and open source software in R. To this end, 
...
For example, zero-inflated and hurdle models are special cases of LCA for dealing data with a very high proportion of zeros (extreme right skew) [@baughman_mixture_2007; @magnus_multidimensional_2022].
...
For instance, a depression questionnaire PH-9 have found 2 distinct groups, ones with no symptoms, and one with symptoms. A mixture model can model the "presence" and "frequency" of symptoms as separate classes, instead of assuming that the sample follows a single continuous underlying distribution [@magnus_multidimensional_2022]. 
...
By default, `tidySEM` derives starting values using K-means clustering [@likas_global_2003; @Forgy1965ClusterAO; @Hartigan_kmeans],
...
$\ln(\frac{n + 2}{24})$. Although simulation studies have shown that the saBIC performs well for class enumeration [@tein_statistical_2013; @chen_efficacy_2017],
a limitation is that its penalty is not based on theory, but on the Minimum Description Length (MDL) as the shortes set of information necessary to reproduce the data [@Rissanen_1985; @sclove_application_1987]. BIC also performs well and is theoretically substantiated.
As such, BIC may be the most appropriate information criterion to use for model comparison [@nylund-gibson_ten_2018; @masyn_latent_2013]. 
...
Thus, a better rule of thumb might be to ascertain that the smallest class has at least as many observations as the average number of parameters per class.
<!-- DA: what is many more? When making a rule a thumb, you have to eyeball a quantity and recommend it for users without statistical intuition
MGV: here I changed it "at least as many", as a low bound recommendation-->
...
For participants assigned to one particular class (rows) according to $M$ (most likely class based on predicted probabilities),
...
By default, `table_fit()` reports the LMR (also called Vuong’s robust likelihood-ratio test, VLMR) for subsequent models in the table.

<!-- DA: We do not introduce VLM, what does it stand for? -->
