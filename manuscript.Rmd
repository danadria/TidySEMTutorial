...
There are several statistical criteria to consider in model selection.
These include but are not limited to information criteria, likelihood ratio tests, 
and the Bayes factor (Weller, Bowen & Faubert, 2020).

Model fit can be assessed through a comparison of multiple information criteria.
...
Further models with additional classes should show worse fit. 
Therefore, information criteria reach an optimum before their values start to rise with increasing number of classes.
...
Other statistical criteria to assess model fit include the likelihood ratio test and the Bayes factor (Weller, Bowen & Faubert, 2020).

The likelihood ratio (LR) $\chi^2$ test is commonly used to assess goodness-of-fit. 
It can be used to compare two nested models.
Two models are said to be nested when one model is based on a subset of indicators of the other model.
Typically, we compare our final class solution, a constrained model to an unconstrained one.
The perfect fit would mean the $\chi^2\ =\ 0$ and $p\ =\ 1$.
We want to the LR test to be non-significant as this implies
that our model is more parsimonious yet it fits the data as well as the unconstrained model.
A downside of the LR $\chi^2$ test statistic is that it is highly dependent on sample size (Masyn, 2013).
Large samples will often effortlessly to reject the null even when the final class solution is a good fit,
and small samples will often be underpowered resulting in non-significant $\chi^2$ statistic.
Special caution is advised when interpreting p-values derived from models with small sample sizes 
or sparse data (Masyn, 2013).
<!-- A potential solution to small sample size biased p values: parametric bootstrapping and posterior predictive checking see Collins & Lanza cited in Masyn 2013) -->

<!-- Add a paragraph about using the Bayes factor -->
...
Once we have selected the final class solution, we have to inspect classification diagnostics.
Classification diagnostics answer the question of how accurate 

Entropy is a measure of accuracy with regards to modeled classes. 
Values above .80 and close to 1 are considered ideal.

The average latent class posterior probability [see Masyn, 2013]
<!-- Continue the paragraph with info about classification diagnostics, e.g. the average latent class posterior probability. -->
