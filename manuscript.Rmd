---
title             : "Best Practices in Latent Class Analysis using the Free Open Source Software"
...
affiliation:
  - id            : "1"
    institution   : "Tilburg University, Methodology & Statistics"
  - id            : "2"
...
this paper introduces new functionality in the R-package `tidySEM` [@van_lissa_tidysem_2022] to perform LCA using free open source software (FOSS).

## The Argument for Free Open Source Software

Despite the popularity of LCA,
most existing software is commercial and closed-source,
for example, *Mplus* [@muthen_mplus_1998], for which the R-package `MplusAutomation` offers augmented LCA functionality [@hallquist_mplusautomation_2018], and *Latent GOLD* [@vermunt_latent_2000].
...
This goes against open science principles [@lamprecht_towards_2020],
...
Notable FOSS for LCA includes `mclust` [@scrucca_mclust_2016], `depmixS4` [@visser_depmixs4_2010], and `OpenMx` [@neale_openmx_2016].
...
<!-- DA: I don't understand the message for `depmixS4`. Why is 'implementing specific innovations' a limitation? If this is a euphemism, we can express the limitation more directly, i.e. qualify these innovations, e.g. [...] which are not software-agnostic (if that is the limitation). -->
`OpenMx` is unique in that, like proprietary *Mplus*, it implements LCA in the context of a fully featured structural equation modeling framework.
A major downside of `OpenMx` is that it is not very user-friendly and its functionality is poorly documented.

These limitations are addressed by the `tidySEM` package,
which provides a user-friendly interface for LCA in `OpenMx`.
...
The primary purpose of the `tidySEM` package is to provide a tidy workflow for generating, estimating, reporting, and plotting structural equation models in a software-agnostic way.
...
For instance, LCA models constructed in `tidySEM` inherit the `MxModel` format from `OpenMx` which renders them compatible with various legacy functions from `OpenMx` and related LCA ecosystem .
Since `tidySEM` was developed with open science in mind, it facilitates fully-reproducible analyses and adheres to the *FAIR (Findable, Accessible, Interoperable, and Reusable) software principles* [@lamprecht_towards_2020],
...
membership [@vermunt_latent_2004].
...
Another way to conceptualize LCA is through an analogy to a measurement model in structural equation modeling [@molenaar_arbitrary_1994].
...
Another difference between the two techniques is that factor analysis can group observed *variables* into latent constructs, while LCA can group *observations* into classes.
...
*unsupervised* learning method for *classification* [@figueiredo_unsupervised_2002].
...
usually binary indicators [@collins_latent_2009].
...
model used, or the level of measurement of the indicators [@vermunt_latent_2004].
To prevent ambiguity, we refer to LCA with binary or ordinal indicators as *LCA with ordinal indicators*,
and use *LCA* as an umbrella term for the family techniques.
...
LCA can be used to identify these four statuses based on observed indicators [e.g., self-reported identity exploration and commitment, @luyckx_developmental_2008].
Similarly, *personality type theory* states that dimensional differences in personality can largely be explained by an undercontrolled, overcontrolled, and resilient personality types, which could also be restored using LCA [@donnellan_resilient_2010].
...
Appendix A showcases a step-by-step tutorial to confirmatory LCA in `tidySEM`,
based on a recent study that tested a theory that identities of Belgian adolescents with migration backgrounds could be summarized as those relating to their heritage, national and regional identities [@maene_perceived_2022].
...
based on the aforementioned study of the distribution of ocean microplastics [@alkema_maximizing_2022].
...
For example, one study found discrepancies between self-reported employment status and official register data [@pavlopoulos_measuring_2015]. 
...
This example also illustrates that LCA allows us to evaluate the reliability of different types of indicators [@geiser_is_2014].
...
see a study on university admission tests of Saudi Arabian students [@tsaousis_measurement_2020].
<!-- LCA revealed three -->
...
For example, one study conducted a LCGA of adolescent empathy development on two dimensions of empathy identifying: high, average, and low empathy classes [@van_lissa_divergence_2015].
...
<!-- DA: this study is not in zotero yet, I lack information (title, authors, etc.) to enter it, @Caspar could you share more info (i.e. the working title, authors, goal journal) enter this to Zotero and adjust the in-text citation-->
...
For example, zero-inflated and hurdle models are special cases of LCA for dealing data with a very high proportion of zeros (extreme right skew) [@baughman_mixture_2007].
...
and one with non-zero symptoms [@magnus_multidimensional_2022].
...
<!-- the aforementioned study on marine plastic noted that many prior studies had treated the distribution of particles as (multivariate) normal, despite clear deviations from normality [@alkema_maximizing_2022].
...
and updated to current best practices showcasing `tidySEM` syntax.
...
```{r checkest-tab, include=TRUE}
item <- data.frame(Item = c(
...
item$`#` = paste0(1:length(item$Item), ".")
knitr::kable(item[, c("#", "Item")], caption = "")
...
The function `descriptives(data)` provides extensive descriptive statistics,
...
Recall that `tidySEM` can estimate mixture models with either numeric (`numeric` or `integer`) or `ordered` type indicators.
The column `type` should thus only contain these types.
Also check that the number of `unique` values match the intended measurement level.
...
response categories are treated as continuous [e.g., @norman_likert_2010; @rhemtulla_when_2012],
it is very unlikely
...
Nominal variables are converted to binary indicators of group membership via dummy coding, for example, using `mx_dummies()`.
...
For example, the `plas_depression` data shows extreme right skew in depressive symptoms:
most respondents report few symptoms,
and few respondents report many symptoms.
Similarly, the `alkema_microplastics` data are extremely right-skewed: Very small plastic fragments abound in the oceans, and large pieces are rare.
...
available information for each subject [@enders_relative_2001; @enders_applied_2022].
...
missingness contingent on *observed* variables that can be known and controlled for (MAR);
and missingness related to unobserved variables,
which can be neither known nor controlled for (MNAR).
A so-called "MCAR" test is often reported [@jamshidian_tests_2010],
...
Our recommendation is to inspect and report the proportion of missingness per variable (e.g., using `tidySEM::descriptives()`),
and report the `mice::mcar()` test, as way to identify variables that can help the parameter recovery instead of evidence of MCAR or MNAR.
As FIML estimation assumes that missingness is either MCAR or MAR,
one would proceed with FIML regardless of the outcome.

One minor concern is that `tidySEM` uses a different algorithm to determine starting values for complete data (K-means) and for missing data (hierarchical clustering).
This should not be a problem, however, as the global optimizer used to estimate the model should be independent of starting values [@corana_minimizing_1987].
...
At present, `tidySEM` only facilitates estimation of models that can be specified in `lavaan` syntax.
...
the default approach in `tidySEM` assumes that each ordinal indicator reflects an underlying standard normal distribution, $N(\mu = 0, \sigma = 1)$.
...
Prior literature emphasized the importance of considering alternative model specifications [@van_de_schoot_grolts-checklist_2017].
...
Most prior literature has emphasized exploratory LCA [e.g., @nylund_deciding_2007].
...
If fit differences are small, the theoretical model may still be preferred, as theory is a relevant consideration in model selection [@jung_introduction_2008].
...
The second consideration in model specification is the risk of overfitting [@hastie_elements_2009].
...
it was observed that if the estimated number of classes exceeded the true number, the number of observations assigned to these classes tended towards zero as sample size increased [@rousseau_asymptotic_2011].
...
In `tidySEM` models can be specified in different ways.
The wrapper functions `mx_profiles()` and `mx_lca()` construct LCA models for continuous and ordinal indicators respectively, without needing to specify model syntax.
The function `mx_profiles()` has two arguments to free elements of the covariance matrix: `variances` and `covariances`.
By default, `mx_profiles()` fixes covariances to zero, and thus performs latent profile analysis.
Finally, function `mx_mixture()` accepts a more flexible `model` argument, which can be a character string specifying a model in `lavaan` syntax,
which is translated to `OpenMx` models via the function `as_ram()`,
or a list of `OpenMx` models.
...
resources [@weller_latent_2020].
LCA in `tidySEM` uses maximum likelihood estimation (ML), which can be conceptually understood as follows:
...
An alternative solution that overcomes these challenges is to use a global optimizer, like *simulated annealing* [*SA*, @corana_minimizing_1987].
...
However, if the destination $-2LL$ is *higher*,
...
then a nonparametric clustering algorithm - like K-means or hierarchical clustering - can be used to determine these cluster centroids [see also @biernacki_choosing_2003].
By default, `tidySEM` derives starting values using K-means clustering [@Hartigan_kmeans],
...
Specify the argument `run = FALSE` in the call to `mx_mixture()` and related functions.
...
The `OpenMx` package contains a family of functions for this purpose: `mxTryHard()` and `mxTryHardWideSearch()` for continuous indicators, and `mxTryHardOrdinal()` for ordinal indicators.
...
<!-- One way to obtain relative evidence in favor of different models is by using BIC weights, obtained by the function `tidySEM::ic_weights()` [@wagenmakers_aic_2004]. -->

<!-- From sinha_practitioners_2021: -->
...
the proportion of cases assigned to the excessive classes converges to zero as the sample size increases [@rousseau_asymptotic_2011].
...
model.
The *Akaike Information Criterion* (*AIC*) is the original information criterion.
...
Finally,
the so-called *sample size adjusted BIC* (*saBIC*), $\ln(\frac{n + 2}{24})$,
implements a penalty for sample size that is based on the Minimum Description Length, a concept from data compression that refers to the smallest number of pieces of information necessary to reproduce the data [@Rissanen_1985; @sclove_application_1987].
Simulation studies have shown that the saBIC performs well for enumeration of homogenous clusters in small samples,
but tends to select too many classes when the number of the number of classes and the sample size are large [@tein_statistical_2013; @chen_efficacy_2017].
...
As such, BIC may be a sensible default criterion for model comparison [@nylund-gibson_ten_2018; @masyn_latent_2013]. 
In `tidySEM`, all three ICs are available as a part of the output of `table_fit()`.
...
which is implemented in `tidyLPA` [@rosenberg_tidylpa_2018].
...
Given a specific information criterion, it is also possible to calculate *IC-weights*,
which indicate the relative support the data provides for each model under consideration [@wagenmakers_aic_2004].
This is particularly useful for confirmatory LCA,
as it allows one to quantify the support for the theoretical model.
In `tidySEM`, IC-weights are calculated by calling `ic_weights()` on the model fit table.
...
When two models are nested, the significance of the difference in fit between these models can be tested using a *Likelihood Ratio* (*LR*) *test*.
...
The so-called *Lo-Mendell-Rubin (LMR) adjusted LR test* overcomes this problem by applying an ad-hoc correction to the LR [@lo_testing_2001].
This test is implemented in `tidySEM`, and is a part of the default output of `table_fit()`.
<!-- A note on ‘Testing the number of components in a normal mixture’ Get access Arrow -->
<!-- Neal O. Jeffries -->
<!-- Biometrika, Volume 90, Issue 4, December 2003, Pages 991–994, https://doi.org/10.1093/biomet/90.4.991 -->
<!-- [LMR]requires conditions that are generally not met when the null hypothesis holds. Consequently, the result is not proven and simulations suggest that it may not be correct. -->
...
and fitting the $k$- and $k-1$-class models to each simulated dataset [@nylund_deciding_2007]. 
...
although both perform comparably well at detecting the true number of classes [@nylund_deciding_2007; @tein_statistical_2013].
...
Instead, specific models can be compared using the function `BLRT()`.

There are known limitations to both aforementioned tests;
on the one hand, the LRT is known to contain mathematical inaccuracies [@jeffriesNoteTestingNumber2003a], yet still shows reasonable performance in simulation studies.
On the other hand, the BLRT is particularly sensitive to model misspecification and is thus not appropriate for exploratory applications [@nylund_deciding_2007].
...
Other authors have reported similar experiences [@sinha_practitioners_2021], which matches the differences between LRT and ICs from information theory as LRT tests and compare the models based on in sample predictive accuracy, and ICs evaluates and compare the models out-of-sample predictive accuracy which helps to control for overfitting [@mcelreath_statistical_2020].
...
A promising novel solution is the so-called *lazy bootstrap*,
...
which describes how well each model is able to reproduce the observed correlation matrix [see @mcelreath_statistical_2020].
A version of the lazy bootstrap is already implemented in the commercial program *Latent GOLD* [@vermunt_upgrade_2021].
...
This is because a model can fit the data well and still exhibit poor separation between the latent classes [@masyn_latent_2013].
...
it makes sense to only consider models which are successful at separating latent classes [@nylund-gibson_ten_2018].
...
In `tidySEM`, individual classification probabilities $P$ are obtained by running `class_prob(res, type = "individual")`.
...
The function `BCH()` does this automatically.
...
This frequency table $F_P$ is obtained by calling `class_prob(res, type = "sum.posterior"`.
...
Second, most likely class membership based on the highest posterior class probability, $F_M$, is obtained by running `class_prob(res, "sum.mostlikely")`.
...
are the minimum and maximum percentage of the sample assigned to a particular class, `n_min` and `n_max`, derived from $F_M$.
This is reported by default because the smallest class proportion, `n_min`, is of particular interest.
When the size of the smallest class is not much larger than the number of parameters per class,
proceed with caution:
The model might not be locally identified in that class [@depaoli_mixture_2013].
...
Well-established rules of thumb suggest ratios of 10 or 20 observations per parameter [@klinePrinciplesPracticeStructural2016; @jacksonRevisitingSampleSize2003].
...
which can be obtained using `class_prob(res, type = "avg.mostlikely")`.
...
such as its values having to be higher than 0.7 [@masyn_latent_2013]. 
...
<!-- Mauricio: Can you fact-check this?!? I really struglle here. MGV: this looks good -->
The minimum and maximum of the diagonal of this table are given by default as a part of the output of `table_fit()` for LCA analyses (`prob_min` and `prob_max`).
...
the entropy criterion [@celeux_entropy_1996].
...
However, overly simplistic and general class names can be misleading, a phenomenon known as the *naming fallacy* [@weller_latent_2020].
...
<!-- its results are stable if the analysis is replicated. -->
This means that the order of classes is arbitrary and may change if the analysis is replicated.
A simple solution to ensure replicable results is setting a random seed before fitting the model by using `set.seed()`.
...
To address this problem, the function `mx_switch_labels()` can be used.
...
At present, the so-called *BCH method* is considered one of the best approaches [@bolck_estimating_2004].
It has been implemented in `tidySEM`. The advantage of the BCH method is that it not only allows for the comparison of individual variables, 
...
and pre-registering study plans prior to data collection or analysis [@nosek_transparency_2016].
...
The `tidySEM` package enables performing LCA in free open source software,
...
and the `worcs` R-package automates many of the steps involved [@van_lissa_worcs_2021].
...
the paper (or analysis section) is written as a *dynamic document* that contains both prose and analysis code.
Results are automatically rendered as text, tables, and plots.
Second,
the research archive is *version controlled*, which means that the entire historical record of the project is retained.
This version controlled archive can be made public.
Third, *dependency management* ensures that all software used is documented, including its version number and provenance.
...
By combining `tidySEM` and `worcs`,
...
The `worcs` package provides a generic, non-parametric method to create a synthetic copy of data when data cannot be shared.
It is also possible to synthesize data directly from the LCA model using `mxGenerateData()`,
...
More interestingly, it also enables creating a *Preregistration As Code* (PAC):
A draft paper with reproducible analysis section based on fake (synthetic or simulated) data [@peikert_reproducible_2021].
...
For an example of PAC, see @van_lissa_complementing_2022.

### Reporting Model Fit

The function `table_fit()` reports an overview of fit indices, customized for LCA.
...
In case of doubt, a full overview of the parameters is given by the `coef()` and `table_results()` functions.
...
By default, `table_fit()` includes the AIC, BIC, and saBIC.
...
By default, `table_fit()` reports the latter as `prob_min` and `prob_max`.
...
the minimum proportion of cases per class can be used to diagnose classes that are too small to be locally identified or practically relevant [see @depaoli_mixture_2013].
By default, `table_fit()` reports the minimum and maximum proportion of cases,
based on most likely class membership.
Fifth, the LMR or BLRT can be reported if it is a part of the class enumeration strategy, using `lr_lmr()` and `BLRT()`.
By default, `table_fit()` reports the LMR for subsequent models in the table.

### Reporting Results

Reporting LCA results involves reporting the class proportions,
obtained via `class_prob(res, "sum.posterior")`,
and model parameters, obtained via `table_results(res)`.
...
Thresholds are more readily interpretable when converted to conditional item probabilities, using the function `table_prob()`.
These indicate the probability of an item being
endorsed given that the observation belongs to a particular latent [@geiser_data_2012].
...
using the function `wald_test()`.
Equality-constrained informative hypotheses are specified using the `bain` syntax,
...
The Grammar of Graphics [@wilkinson_grammar_2005; @wickham_layered_2010] 
addresses best practices in visualization, 
which we summarize here with specific emphasis on the application to LCA.
Most importantly, good graphics start with the *data* in mind.
...
A second consideration are which *aesthetics* to use.
...
it makes sense to *facet* a plot into multiple panels by class solution.
The plots in `tidySEM` are based on best grammar of graphics practices,
and are implemented in `ggplot2` (@wickham_ggplot2_2016).
As such, these plots can be further modified just like any other `ggplot2` object.
One remaining limitation is that these best practices have only been implemented for LCA with continuous indicators, in `plot_profiles()`, `plot_density()` and `plot_bivariate()`, and for LCGA in `plot_growth()`. Plots for LCA with ordinal indicators are still rudimentary, more work remains to be done in this area.
...
Fourth, previous reporting standards called for reporting of sample descriptive statistics [see @schreiber_latent_2017].
...
Gaussian mixture models assume that the distribution of observed indicators is a *mixture of multiple normal distributions*.
<!-- In our shoe -->
...
whereby additional models are estimated until the selected class enumeration criteria no longer show improvement of model fit [e.g., see @spurk_latent_2020].
...
(1) Providing a comprehensive primer of LCA models
...
(2) Providing guidance with respect to best practices in estimation and reporting of mixture models.
...
(3) Creating a simultaneously most powerful and simple R package for fitting LCA models
